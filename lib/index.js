...
Assertion.prototype.all = function(msg){
  ...
  channels.forEach(function(channel){
    var err = fmt('expected integration to be enabled on all channels its disabled on "%s"', channel);
    msg.channel = channel;
    assert(self.enabled(msg), err);
  });
  return this;
};
...
channels.forEach(function(channel){
  Assertion.prototype[channel] = function(obj){
    obj = obj || {};
    obj.channel = channel;
    var msg = fmt('expected integration to be enabled for "%s"', channel);
    assert(this.enabled(obj), msg);
    return this;
  };
});
