...
Assertion.prototype.fixture = function(name){
  ...
  return require(join(this.dirname, 'fixtures', name + '.json'));
};
...
Assertion.prototype.ensure = function(path, expected){
  var actual = this.integration.constructor.validations[path];
  if (!meta) throw errorf('expected integration to have validation for "%s"', path);
  if ('function' == typeof meta) return;

  // clean
  actual = Object.create(actual);
