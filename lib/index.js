...
/**
 * Assert that the integration errors with optional error `msg` and/or `status`.
 *
 * @param {Number} [status]
 * @param {String} [msg]
 * @param {Function} fn
...
Assertion.prototype.error = function(){
  ...
  this.end(function(err, responses){
    if (!err) {
      return fn(new Error('expected integration to error'), responses);
    }

    if (msg && err.message !== msg) {
      return fn(utils.equals(err.message, msg))
    }

    if (status && err.status !== status) {
      return fn(utils.equals(err.status, status))
    }
